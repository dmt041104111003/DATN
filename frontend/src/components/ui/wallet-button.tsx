...
function WalletButton({ className, children = "Wallet", icon, ...props }: WalletButtonProps) {
  return (
    <>
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          .wallet-card:hover {
            transform: translateY(-2px) scale(1.002) translateZ(0);
            box-shadow: 0 24px 36px rgba(0, 0, 0, 0.11),
              0 24px 46px var(--box-shadow-color);
          }
          .wallet-card:hover .wallet-overlay {
            transform: scale(4) translateZ(0);
          }
          .wallet-card:hover .wallet-circle {
            border-color: var(--bg-color-light);
            background: var(--bg-color);
          }
          .wallet-card:hover .wallet-circle:after {
      ...
      <button
        type="button"
        className={cn("wallet-button", className)}
        {...props}
      >
        <div className="wallet-card">
          ...
          <p className="wallet-text">{children}</p>
        </div>
      </button>
    </>
  )
