...
export function Header() {
  ...
  const [productDropdownContent, setProductDropdownContent] = useState<DropdownContent | null>(null);
  ...
  useEffect(() => {
    ...
    fetch('/api/categories')
      ...
      .then((list: ApiCategory[]) => {
        ...
        setProductDropdownContent({
          leftColumn: [],
          rightColumn: list.map((c) => ({
            id: slugify(c.labelEn),
            label: { en: c.labelEn, vi: c.labelVi, zh: c.labelZh, fr: c.labelFr },
  ...
  const activeProductType =
    pathSegments[0] === 'products' && pathSegments[1] && pathSegments[2]
      ? pathSegments[1]
      : null;

  const searchParams = useSearchParams();
  const activeProductTypeFromQuery = pathname === '/products' ? searchParams.get('type') : null;
  const activeProductTypeResolved = activeProductTypeFromQuery ?? activeProductType;

  const menuItems = MENU_ITEMS.map((item) =>
    item.id === 'products' && productDropdownContent
      ? { ...item, dropdownContent: productDropdownContent }
      : item
  );
  ...
  const handleNavClick = (itemId: string) => {
    ...
    if (itemId === 'support') {
      if (pathname === '/') {
        scrollToSection('support-preview');
      } else {
        router.push('/#support-preview');
      }
      setOpenDropdown(null);
      closeDrawer();
      return;
    ...
    if (itemId === 'team') {
      if (pathname === '/') {
        scrollToSection('team-preview');
      } else {
        router.push('/#team-preview');
      }
      setOpenDropdown(null);
      closeDrawer();
      return;
                ...
                <div key={item.id} className="relative">
                  <button
                    onClick={() => {
                      ...
                      } else if (item.id === 'home' || item.id === 'support' || item.id === 'team') {
        ...
        <>
          ...
          <div className="fixed top-0 right-0 z-50 w-full h-full bg-white dark:bg-gray-800 md:hidden flex flex-col header-drawer-right">
            ...
            <nav className="flex-1 p-4 pt-0 flex flex-col gap-1 overflow-auto">
                    ...
                    {item.hasDropdown && mobileExpandedDropdown === item.id && item.dropdownContent && (
                      <div className="pl-4 pr-4 pb-2 flex flex-col gap-1">
                        ...
                        {item.dropdownContent.rightColumn.map((sub) => {
                          const isActiveType = item.id === 'products' && sub.id === activeProductTypeResolved;
                          return (
                            <button
                              key={sub.id}
              ...
              <div className="mt-3 px-2 py-3 flex flex-wrap items-center gap-2">
                <ThemeToggle />
                <LanguageToggle />
              </div>
            </nav>
            ...
            <div className="flex border-t border-gray-200 dark:border-gray-700">
              ...
              <div className={`p-4 ${menuItems.find(item => item.id === openDropdown)?.dropdownContent?.leftColumn.length ? 'flex-[2]' : 'flex-1'}`}>
                {menuItems.find(item => item.id === openDropdown)?.dropdownContent?.rightColumn.map((rightItem) => {
                  const isActiveType = openDropdown === 'products' && rightItem.id === activeProductTypeResolved;
                  return (
                    <button
                      key={rightItem.id}
                      onClick={() => {
