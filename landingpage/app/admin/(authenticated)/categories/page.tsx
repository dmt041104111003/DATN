...
import { getCategoryLocale, getCommonLocale } from '../../constants/locale';
...
export default function CategoriesPage() {
  ...
  const t = getCategoryLocale(adminLang);
  ...
  return (
    <>
      ...
      <div className={styles.pageHeader}>
        ...
        <button type="button" className={styles.addIcon} onClick={openAdd} aria-label={t.addCategory} title={t.addCategory}>+</button>
      </div>
          ...
          <tbody>
            {paginatedList.map((c) => (
              <tr key={c.id}>
                ...
                <td>{c.imageUrl ? t.hasImage : '—'}</td>
                <td>{c.sortOrder}</td>
                <td>
                  <div className={styles.actions}>
                    <button type="button" className={styles.btnSecondary} onClick={() => openEdit(c)}>{t.edit}</button>
                    <button type="button" className={styles.btnDanger} onClick={() => handleDelete(c.id)}>{t.delete}</button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div className={styles.tableCards}>
        {paginatedList.map((c) => (
          <div key={c.id} className={styles.tableCard}>
            ...
            <div className={styles.tableCardRow}>
              <span className={styles.tableCardLabel}>{t.description}</span>
              <span className={styles.tableCardValue}>{c.descriptionVi ?? '—'}</span>
            </div>
            <div className={styles.tableCardRow}>
              <span className={styles.tableCardLabel}>{t.image}</span>
              <span className={styles.tableCardValue}>{c.imageUrl ? t.hasImage : '—'}</span>
            </div>
            <div className={styles.tableCardRow}>
        ...
        <div className={styles.dialogBackdrop} onClick={() => setOpen(false)}>
          <div className={styles.dialogPanel} onClick={(e) => e.stopPropagation()}>
            <div className={styles.dialogHeader}>
              <h2 className={styles.dialogTitle}>{editingId ? t.editCategory : t.addCategoryTitle}</h2>
              <button type="button" className={styles.dialogClose} onClick={() => setOpen(false)} aria-label={t.close}>
                ×
              </button>
            </div>
