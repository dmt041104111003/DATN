...
function mapApiProductToItem(p: {
  ...
  descriptionBlocks?: Array<{ titleEn?: string; titleVi?: string; titleZh?: string; titleFr?: string; textEn?: string; textVi?: string; textZh?: string; textFr?: string; imageUrl?: string | null; youtubeUrl?: string | null }> | null;
  imageUrl: string | null;
  youtubeUrl?: string | null;
  category?: { labelEn: string; labelVi: string; labelZh: string; labelFr: string };
}): ProductItem {
  const blocks = p.descriptionBlocks && Array.isArray(p.descriptionBlocks) && p.descriptionBlocks.length > 0
    ? p.descriptionBlocks.map((b) => ({
        title: {
          en: b.titleEn ?? '',
          vi: b.titleVi ?? '',
          zh: b.titleZh ?? '',
    ...
    : undefined;
  const singleDesc = blocks === undefined ? {
    en: p.descriptionEn ?? '',
    vi: p.descriptionVi ?? '',
    zh: p.descriptionZh ?? '',
    fr: p.descriptionFr ?? '',
