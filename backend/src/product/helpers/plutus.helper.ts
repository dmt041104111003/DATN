...
export class PlutusHelper {
  ...
  constructor() {
    ...
    console.log(`[Product.PlutusHelper] Loaded ${this.plutusJson?.validators?.length || 0} validators`);
  }
  ...
  getScripts(owners: string[]) {
    ...
    const contractAddress = serializePlutusScript(spendScript, undefined, this.appNetworkId, false).address;
