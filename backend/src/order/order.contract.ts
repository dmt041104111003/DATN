...
export class OrderContract {
  ...
  constructor(opts: OrderContractOpts = {}) {
    ...
    this.validatorTitle = opts.validatorTitle ?? SCRIPT_VALIDATOR_TITLE;
  ...
  private getValidator() {
    ...
    if (!v) v = this.plutus.validators.find((x) => x.title === SCRIPT_VALIDATOR_TITLE);
    if (!v) v = this.plutus.validators.find((x) => x.title === SCRIPT_VALIDATOR_TITLE_ALT);
    if (!v) throw new Error(`Validator ${this.validatorTitle} not found in plutus.json`);
    return v;
  }
  ...
  buildDatum(d: OrderDatum): { alternative: number; fields: [string[], number, string] } {
    return {
      alternative: 0,
  ...
  async parseDatumFromUtxo(utxo: UTxO): Promise<OrderDatum> {
