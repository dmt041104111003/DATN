...
let DashboardService = class DashboardService {
    ...
    async countPendingVerification(custodian, productionKeys, containerKeys, storageIds) {
        const or = [];
        ...
        if (!or.length)
            return 0;
        return await this.prisma.recordOperation.count({
            where: { verified: false, OR: or },
        });
    }
    async loadVerifiedContainerKeys(containerKeys) {
    ...
    async buildWarehouseStats(custodian, includeConsume) {
        const warehouses = await this.prisma.warehouse.findMany({
            where: { registeringCustodianAddress: custodian },
            select: { id: true, name: true, capacity: true },
        });
        const warehouseIds = warehouses.map((w) => cleanString(w.id)).filter(Boolean);
        ...
        const containerKeys = storageRows
            .map((r) => cleanString(r.containerInventoryKey))
            .filter(Boolean);
        ...
        const insByDay = new Map(dayKeys.map((d) => [d, 0]));
        for (let i = 0; i < storageRows.length; i += 1) {
            ...
            if (insByDay.has(dk))
                insByDay.set(dk, (insByDay.get(dk) || 0) + 1);
        }
        let consumedToday = 0;
        let consumedLast7Days = 0;
        let consumedTotal = 0;
        const consumeByDay = new Map(dayKeys.map((d) => [d, 0]));
        if (includeConsume && storageRows.length) {
            const storageIds = storageRows.map((r) => cleanString(r.id)).filter(Boolean);
            ...
            for (let i = 0; i < consumeOps.length; i += 1) {
                ...
                if (createdAt >= weekStart)
                    consumedLast7Days += 1;
                const dk = toDateKey(createdAt);
                if (consumeByDay.has(dk))
                    consumeByDay.set(dk, (consumeByDay.get(dk) || 0) + 1);
            }
        }
        ...
        const utilizationPercent = totalCapacityKg > 0 ? Math.min(100, Math.round((usedKg / totalCapacityKg) * 100)) : 0;
        return {
            ...
            activityByDay,
            storageIds: storageRows.map((r) => cleanString(r.id)).filter(Boolean),
        };
    }
    async buildEnterpriseStats(custodian) {
        const productions = await this.prisma.production.findMany({
            where: { registeringCustodianAddress: custodian },
            select: { inventoryKey: true, status: true, actualYieldKg: true },
        });
        const productionKeys = productions.map((p) => cleanString(p.inventoryKey)).filter(Boolean);
        ...
        const containerKeys = containers.map((c) => cleanString(c.inventoryKey)).filter(Boolean);
        ...
        const inStorageRows = containerKeys.length
            ...
            : [];
        const inStorageSet = new Set((inStorageRows || []).map((r) => cleanString(r.containerInventoryKey)).filter(Boolean));
        let verified = 0;
        let consumed = 0;
        let inStorage = 0;
        let totalWeightKg = 0;
        ...
        const pendingVerification = await this.countPendingVerification(custodian, productionKeys, containerKeys, warehouseStats.storageIds);
        return {
            role: 'ENTERPRISE',
            production: {
    ...
    async buildLogisticsStats(custodian, role) {
    ...
    async getStats(custodianRaw, roleRaw) {
        const custodian = cleanString(custodianRaw);
        const role = cleanString(roleRaw).toUpperCase();
        if (!custodian)
            throw new Error('Không xác định được ví đăng nhập.');
        if (role === 'ENTERPRISE')
            return this.buildEnterpriseStats(custodian);
        if (role === 'AGENT')
            return this.buildLogisticsStats(custodian, 'AGENT');
